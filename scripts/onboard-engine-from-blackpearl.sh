...
scp "${SSH_OPTS[@]}" "$REPO/scripts/join-k3s-worker.sh" "$REPO/scripts/setup-engine-access.sh" "$ENGINE_HOST:~/k3s-join/"

echo "==> join k3s agent on engine (needs sudo on engine)"
ssh -t "${SSH_OPTS[@]}" "$ENGINE_HOST" "chmod +x ~/k3s-join/join-k3s-worker.sh && sudo K3S_URL=$K3S_URL K3S_TOKEN='$TOKEN' NODE_NAME=engine ~/k3s-join/join-k3s-worker.sh"
