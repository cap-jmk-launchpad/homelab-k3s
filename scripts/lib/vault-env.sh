#!/usr/bin/env bash
# Vault env helpers — preserve credentials across routine deploys.
#
...
load_vault_env() {
  ...
  if [[ -n "$env_file" ]]; then
    while IFS= read -r line || [[ -n "$line" ]]; do
      ...
      val="${val%\'}"; val="${val#\'}"
      case "$key" in
        VAULT_ADDR|VAULT_TOKEN|VAULT_ROOT_TOKEN|VAULT_UNSEAL_KEY|VAULT_NAMESPACE|HCP_CLIENT_ID|HCP_CLIENT_SECRET) ;;
        *) continue ;;
      esac
      [[ -n "$val" ]] && saved["$key"]="$val"
    done <"$env_file"
  ...
  # Bootstrap token alias
  export VAULT_TOKEN="${VAULT_TOKEN:-${VAULT_ROOT_TOKEN:-}}"
  if [[ "${VAULT_ADDR:-}" == *hashicorp.cloud* ]]; then
