...
The dashboard JSON lives in git but is **not** bundled with Loki or kube-prometheus-stack Helm charts. It must be loaded as a ConfigMap with label `grafana_dashboard=1` (sidecar auto-import).

1. On **blackpearl**, from this repo: `bash scripts/homelab-deploy-dashboards.sh`
2. Wait ~60s for the Grafana sidecar to reload, then open:
   `http://192.168.10.41:30300/d/homelab-pod-logs/homelab-pod-logs`
3. Verify ConfigMaps: `kubectl get cm -n monitoring -l grafana_dashboard=1` � expect `homelab-logs-dashboard` among others.
4. API check (optional): `curl -s -u admin:PASSWORD http://192.168.10.41:30300/api/dashboards/uid/homelab-pod-logs | head`

`homelab-deploy-logging.sh` runs the dashboard script automatically; if you deployed Loki before that change, re-run `homelab-deploy-dashboards.sh` once.

**Dashboard loads but panels are empty ("No data")**

This is different from "Dashboard not found". The dashboard exists; Loki, Alloy, or the Loki datasource is missing or has no log lines yet. Deploy logging with `bash scripts/homelab-deploy-logging.sh` and ensure the Grafana Helm values include the Loki datasource (`kube-prometheus-stack-values.yaml` ? `additionalDataSources`).
**No logs in Grafana**
...
| [homelab-deploy-dashboards.sh](../scripts/homelab-deploy-dashboards.sh) | Grafana dashboard ConfigMaps (sidecar) |
| [homelab-deploy-logging.sh](../scripts/homelab-deploy-logging.sh) | Loki + Alloy deploy (includes dashboards) |
