...
[SigNoz](https://signoz.io/) provides logs, traces, and metrics in one UI with OpenTelemetry-native ingestion. It runs alongside the existing **Prometheus + Grafana** metrics stack and **Loki + Alloy** logs during transition.
...
**Still running (transition):** Grafana Alloy → Loki on engine ([homelab-logging.md](./homelab-logging.md)). Same pod logs may appear in both Loki and SigNoz until you turn off one path.

**desktop (WSL worker):** k8s-infra log agents mount host `/` and may fail with `path "/" is mounted on "/" but it is not a shared or slave mount` (same class of issue as some hostPath collectors on WSL). Logs from desktop pods are still available via Alloy→Loki and other nodes' agents; fix is optional (mount propagation on WSL or exclude desktop from the DaemonSet).

## Migration from Loki

| Phase | Action |
|-------|--------|
| **Now (dual)** | Keep Loki + Alloy; use SigNoz for OTel traces and unified triage. Compare log UX in both UIs. |
| **Reduce duplication** | Disable `presets.logsCollection` in k8s-infra *or* stop Alloy `loki.source.kubernetes` — pick one log pipeline. |
| **Rip-and-replace** | `helm uninstall loki -n monitoring`; remove Alloy DaemonSet; drop Loki datasource from Grafana values. Keep Prometheus for infra metrics. |

Recommended homelab path: run **dual** for 1–2 weeks, then disable Alloy log collection and keep Loki only if you still need LogQL/Grafana dashboards; otherwise uninstall Loki to free engine HDD.

### Optional: Alloy → OTLP instead of k8s-infra logs
